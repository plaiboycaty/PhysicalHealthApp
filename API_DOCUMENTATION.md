...
- **Header:** **Tùy chọn (Optional Auth)**. Nếu gửi Token sẽ lưu vào DB. Nếu không gửi Token sẽ được tính là Khách (Guest) và không lưu vào DB.
...
```
- **Response:** 
  - Cần bắt cờ `treatment_status` (`healthy`, `treatment`, `emergency`) để bật Modal.
  - Sẽ có cờ `"is_guest": true` nếu người dùng nộp bài dưới dạng Khách. Dùng cờ này để bật popup *"Hãy đăng ký để nhận lộ trình cá nhân hóa"*.
